...
<% if (session.getAttribute("user") != null && request.getAttribute("hiddenAlbumList") != null) { %>
<!doctype html>
...
<main class="w-100 m-auto form-container">
	...
	<table class="table">
		...
		<tbody>
		<%
			List<Album> hiddenAlbumList = (List<Album>) request.getAttribute("hiddenAlbumList");
			for (Album album : hiddenAlbumList) {
		%>
		<tr>
			<td><%= album.getAlbumname() %></td>
			<td><a href="list-more-details-albuns.jsp?albunId=<%= album.getUuid() %>" class="btn btn-success btn-sm"> Mais detalhes </a></td>
			<td>
